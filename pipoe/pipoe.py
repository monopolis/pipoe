...
def get_package_dependencies_from_remote(info, version):
    """ Looks up requires_dist from an actual package """


    pkg_versions = info["releases"].get(version)
    if not pkg_versions:
        return
...
def get_package_info(
    ...
    try:
        ...
        if requires_dist is None:
            requires_dist = get_package_dependencies_from_remote(info, version)
